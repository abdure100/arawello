...
### Control UI chat not working (Send does nothing or errors)

If the dashboard loads but chat sends fail or nothing happens:

1. **Connected** — The UI must show "Connected". If it shows "Disconnected" or "Connecting...", Send is disabled. Fix: set the correct gateway URL (e.g. `ws://127.0.0.1:18789` when using an SSH tunnel), paste the gateway token from the server `.env` or config, and for remote HTTP use `gateway.controlUi.allowInsecureAuth: true` with a token.
2. **LLM/API keys on server** — Chat runs the agent, which needs a model provider. For Docker deployments, ensure the container has `OPENAI_COMPAT_API_KEY` (and `OPENAI_COMPAT_BASE_URL` if using a custom endpoint) in the server `.env`. The deploy script copies these from your local `.env`; add them there and redeploy, or add them manually in `~/openclaw/.env` on the server and restart the container.
3. **Errors in UI** — A red callout or an assistant message starting with "Error:" shows the failure (e.g. missing API key, "send blocked by session policy"). Check the browser devtools console for WebSocket or `chat.send` request failures.
4. **Gateway logs** — On the server run `docker compose logs -f openclaw-gateway` (or `openclaw logs --follow` if not Docker). Look for `chat.send` lines and any errors after them (e.g. `MissingEnvVarError: OPENAI_COMPAT_API_KEY`).
