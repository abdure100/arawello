...
## Dashboard shows Offline or locked (cannot change anything)

The UI only allows editing when the WebSocket to the gateway is **Connected**. If you see **Health: Offline** or **Disconnected** in the Snapshot card, the rest of the dashboard stays read-only.

1. Open the **Overview** tab and check **Gateway Access**.
2. Ensure **WebSocket URL** matches how you opened the page: e.g. `ws://localhost:18789` when using `http://localhost:18789`. The UI now auto-corrects this when you load the page from localhost so a stale saved URL does not block connection.
3. With `gateway.controlUi.allowInsecureAuth: true`, you can leave the token empty when using the same origin (e.g. localhost). Click **Connect**.
4. If you see an error in the red callout (e.g. "device identity required" or "unauthorized"), rebuild the gateway image so it includes same-origin bypass, then recreate the container: `docker build -t openclaw:local .` and `docker compose up -d openclaw-gateway --force-recreate`.
5. Use a single dashboard tab; close others and refresh so one connection is active.
