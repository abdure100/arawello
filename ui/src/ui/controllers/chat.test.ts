...
describe("handleChatEvent", () => {
  ...
  it("processes final from own run and clears state", () => {
    ...
    expect(state.lastError).toContain("No response from model");
  });

  it("sets lastError when final has no message, uses emptyHint when provided", () => {
    ...
    expect(state.lastError).toBe("Custom hint");
  });

  it("clears lastError when final includes assistant message", () => {
    const state = createState({
      sessionKey: "main",
      chatRunId: "run-1",
