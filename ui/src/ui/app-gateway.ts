...
export function connectGateway(host: GatewayHost) {
  ...
  host.gatewayConnecting = true;
  host.execApprovalQueue = [];
  host.execApprovalError = null;

  host.client?.stop();
  // Clear connecting state if neither onHello nor onClose fires within 15s (e.g. WS never connects)
  const connectingTimeout = window.setTimeout(() => {
    if (host.gatewayConnecting) {
      host.gatewayConnecting = false;
    }
  }, 15_000);
