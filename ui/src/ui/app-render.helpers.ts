...
export function renderTab(state: AppViewState, tab: Tab) {
  ...
  const handleClick = (event: MouseEvent) => {
    // Allow modifier keys for native behavior (e.g. open in new tab)
    ...
    if (state.tab !== tab) {
      state.setTab(tab);
      // Force update in case Lit didn't pick up the state change
      if (
        "requestUpdate" in state &&
        typeof (state as { requestUpdate: () => void }).requestUpdate === "function"
      ) {
