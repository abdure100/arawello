...
export function applySettingsFromUrl(host: SettingsHost) {
  // When gateway serves the UI with allowInsecureAuth, it injects the token. Prefer it over localStorage
  // so a wrong/stale saved token does not block connection (token_mismatch).
  if (typeof window !== "undefined") {
    const injected = window.__OPENCLAW_GATEWAY_TOKEN__?.trim();
    if (injected && injected !== host.settings.token?.trim()) {
      applySettings(host, { ...host.settings, token: injected });
    }
    // If we're on localhost/127.0.0.1, ensure gatewayUrl matches so same-origin connection works
    // (stale localStorage URL from another host would otherwise keep the UI disconnected/locked).
    const originHost = window.location.hostname.toLowerCase();
    const isLocal = originHost === "localhost" || originHost === "127.0.0.1";
    if (isLocal) {
      const proto = window.location.protocol === "https:" ? "wss" : "ws";
      const expectedUrl = `${proto}://${window.location.host}`;
      if (host.settings.gatewayUrl !== expectedUrl) {
        applySettings(host, { ...host.settings, gatewayUrl: expectedUrl });
      }
    }
  }
