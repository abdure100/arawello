...
export type ToolsConfig = {
  ...
  /** FileMaker (or compatible) semantic search endpoint. Uses FileMaker security and indexing; agent calls this for client notes, claims, policies. */
  filemaker?: {
    /** Enable filemaker_search tool (default: false). */
    enabled?: boolean;
    /** Base URL for the search API (e.g. Laravel or FileMaker Data API proxy). Must accept POST with JSON body { query, maxResults? }. */
    baseUrl?: string;
    /** API key or bearer token (optional; can use Authorization header from headers instead). */
    apiKey?: string;
    /** Timeout in seconds for search requests (default: 15). */
    timeoutSeconds?: number;
    /** Default max results to return (default: 10). */
    maxResults?: number;
    /** Optional extra headers (e.g. X-API-Key, Authorization). */
    headers?: Record<string, string>;
  };
