...
/** True if request is from a page we served (same-origin Control UI). Used with allowInsecureAuth to accept without token. */
function isSameOriginControlUi(
  ...
  const originOk =
    origin === `http://localhost:${portStr}` || origin === `http://127.0.0.1:${portStr}`;
  const hostOk = host === `localhost:${portStr}` || host === `127.0.0.1:${portStr}`;
  ...
  socket.on("message", async (data) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // When allowInsecureAuth is true, accept Control UI from same-origin without any token/password.
        // The page was just served by this gateway; no need for the UI to know or send the token.
        let sameOriginBypass = false;
        const port = resolveGatewayPort(configSnapshot);
        if (
          isControlUi &&
          allowInsecureControlUi &&
          isSameOriginControlUi(requestOrigin, requestHost, port)
        ) {
          authOk = true;
          authMethod = "token";
        ...
        // When allowInsecureAuth is set, token/password auth can skip device pairing (CLI + Control UI).
        const allowTokenWithoutPairing =
          configSnapshot.gateway?.controlUi?.allowInsecureAuth === true && sharedAuthOk;
        const skipPairing = (allowControlUiBypass || allowTokenWithoutPairing) && sharedAuthOk;
