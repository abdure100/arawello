...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    try {
      ...
      // Register so server-chat agent event handler can map evt.runId -> clientRunId/sessionKey.
      ...
      const preview = (
        trimmedMessage.length > 80 ? `${trimmedMessage.slice(0, 80)}…` : trimmedMessage
      )
        .replace(/\s+/g, " ")
        .trim();
      context.logGateway.info(
        `chat.send: sessionKey=${p.sessionKey} runId=${clientRunId} attachments=${normalizedAttachments.length} message=${JSON.stringify(preview)}`,
      );
