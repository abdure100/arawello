...
export type AgentEventHandlerOptions = {
  ...
  /** Optional: log when chat final is sent with no message (helps debug missing replies). */
...
export function createAgentEventHandler({
  ...
  const emitChatFinal = (
    ...
    if (jobState === "done") {
      if (!text && logGateway) {
        logGateway.debug(
          `chat final with empty buffer: clientRunId=${clientRunId} sessionKey=${sessionKey} (assistant event may not have been emitted or had empty text)`,
        );
      }
      const payload = {
        runId: clientRunId,
