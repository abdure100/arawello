...
export function subscribeEmbeddedPiSession(params: SubscribeEmbeddedPiSessionParams) {
  ...
  /** Emit assistant from lastAssistant (if any) or fallbackText, then lifecycle "end". Call from attempt after run so chat gets reply when provider never streamed. */
  const emitLifecycleEnd = (
    lastAssistant?: { role?: string; content?: unknown } | null,
    fallbackText?: string,
  ) => {
    let text = "";
    if (lastAssistant?.role === "assistant") {
      text = extractAssistantText(lastAssistant as AssistantMessage)?.trim() ?? "";
    }
    ...
    if (text) {
      ...
      void params.onAgentEvent?.({ stream: "assistant", data: { text } });
    }
    emitAgentEvent({
      runId: params.runId,
      stream: "lifecycle",
    ...
    void params.onAgentEvent?.({ stream: "lifecycle", data: { phase: "end" } });
  };
