...
export async function resolveApiKeyForProvider(params: {
  ...
  if (customKey) {
    // Config may hold env var name (e.g. OPENAI_COMPAT_API_KEY); resolve from process.env when set.
    const valueFromEnv = process.env[customKey]?.trim();
    if (valueFromEnv) {
      return { apiKey: valueFromEnv, source: `env: ${customKey}`, mode: "api-key" };
    }
    // If it looks like an env var name (e.g. OPENAI_COMPAT_API_KEY), don't send it literally — provider would 401.
    if (/^[A-Z][A-Z0-9_]*$/.test(customKey)) {
      throw new Error(
        `Custom provider "${provider}" is configured with apiKey "${customKey}" but that environment variable is unset or empty. Set it in the process that runs the gateway (e.g. export ${customKey}=your-key).`,
      );
    }
